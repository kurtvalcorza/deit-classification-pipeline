...
FILES = {
    ...
    "pytorch_model.bin": b"\x00" * 64,
}
REFERENCE = {"tf_model.h5": b"\x01" * 80}
...
def test_pin_writes_commit_digests_and_module_revision(tmp_path):
    ...
    manifest = json.loads((root / "weights/deit-small-patch16-224/dimer-base-manifest.json").read_text())
    ...
    module = (root / "src/deit_classification_pipeline/pipeline.py").read_text()
    assert f'MODEL_REVISION = "{COMMIT}"' in module
    [reference] = manifest["referenceFiles"]
    assert (
        reference["sha256"] == hashlib.sha256(REFERENCE["tf_model.h5"]).hexdigest()
        and reference["bytes"] == 80
    )
    assert "tf_model.h5" not in {name for name, _revision in calls}  # recorded, never downloaded


def test_pin_refuses_a_digest_that_disagrees_with_the_hub(tmp_path):
    root = _copy_repo(tmp_path)
    before = (root / "weights/deit-small-patch16-224/dimer-base-manifest.json").read_text()
    module_before = (root / "src/deit_classification_pipeline/pipeline.py").read_text()
    model_info, download, _calls = _fake_hub(FILES, lfs_override={"pytorch_model.bin": "f" * 64})
    assert pin_snapshot.pin(root, model_info=model_info, download=download) == 1
    assert (root / "weights/deit-small-patch16-224/dimer-base-manifest.json").read_text() == before
    assert (root / "src/deit_classification_pipeline/pipeline.py").read_text() == module_before
